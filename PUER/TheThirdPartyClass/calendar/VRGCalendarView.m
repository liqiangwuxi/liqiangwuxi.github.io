...
#pragma mark - 跳转到前一天或后一天
-(void)resetaa {
    
    //获取前一天的时间
    NSString *dateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"currentDate"];
    NSDateFormatter  *dateformatter=[[NSDateFormatter alloc] init];
    [dateformatter setDateFormat:@"yyyy-MM-dd"];
    NSDate *  senddate=[dateformatter dateFromString:dateString];
    
    self.currentMonth = senddate;
...
-(void)selectDate
{
    //获取当前时间
    NSString *dateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"currentDate"];
    int date = [[dateString substringFromIndex:8] intValue];
    [self selectDate:date];
    return;
}

- (void)theDayBeforeOrAfter
{
    [self resetaa];
    [self selectDate];
...
-(id)init {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(theDayBeforeOrAfter) name:@"theDayBeforeOrAfter" object:nil];
    ...
    if (self) {
        ...
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(34, 0, kVRGCalendarViewWidth-68, 30)];
        [self addSubview:labelCurrentMonth];
        labelCurrentMonth.backgroundColor=[UIColor whiteColor];
        labelCurrentMonth.font = [UIFont fontWithName:@"Helvetica-Bold" size:17];
