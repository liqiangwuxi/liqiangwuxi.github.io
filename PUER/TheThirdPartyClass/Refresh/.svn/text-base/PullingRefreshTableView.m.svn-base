...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
  ...
  //  NSLog(@"yMargin:%f\noffset.y:%f\nsize.height:%f\ncontentsize.height%f",yMargin,offset.y,size.height,contentSize.height);
...
//            NSLog(@"kpprrrrr");
            if(contentSize.height < size.height)
            {
//                NSLog(@"%f--%f--%f",contentSize.height,size.height,offset.y);
                self.contentInset = UIEdgeInsetsMake(0,0,size.height-contentSize.height+kPROffsetY,0);
            }else{
